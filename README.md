...
- `loan_to_income`, `monthly_burden`, `high_dti_flag`, `long_term_flag`, `cost_of_credit`, `risk_interaction`

**Behavioral / categorical encodings**
- `loan_size` (Micro/Small/Medium/Large), `credit_tier` (Poor → Exceptional), `income_segment` (Low → High)

**India / Emerging Market Context (synthetic, grounded)**
- `digital_onboarding`, `upi_transaction_count`, `mobile_credit_score`, `first_time_borrower`, `urban_flag`

**Targets**
- `default` (binary), `risk_tier` (Low/Medium/High), `expected_loss` (continuous)
...
### 5.1 — Optimal Approval Threshold
Swept default-probability thresholds from 0.01 to 0.99, simulating approval/profit outcomes at each.

| Metric | Value |
|---|---|
| **Optimal Threshold** | 0.78 |
| Approval Rate | 94.9% |
| Resulting Default Rate | 1.3% |
| Estimated Profit | **$167,185,678** |

> At the portfolio level, a loose 0.78 threshold maximizes profit while keeping realized defaults at 1.3% — far below the unconditional 4% baseline, because high-probability defaulters are concentrated in a small fraction of loans.
...
The 255MB SQLite database is hosted on Google Drive and downloaded at runtime via `gdown` on server startup.
...
| `/early-warning` | GET | Top 100 flagged loans with risk details |
| `/shap` | GET | SHAP feature importance rankings |

### Example `/predict` Response
```json
{
  "approval": { "approved": true, "decision": "APPROVED", "confidence": 0.284 },
  "risk": {
    "default_probability": 0.7164,
    "risk_tier": "Low",
...
> **Known limitation**: `risk_tier` is produced by a separately trained classifier from `default_probability`, so the two can occasionally disagree (as in the example above — 71.6% probability labeled "Low Risk"). Documented in [Future Improvements](#scope-for-fine-tuning--future-improvements) with the fix.
...
**Model Level**
- **Fix risk_tier inconsistency** — derive `risk_tier` directly from `default_probability` thresholds (e.g. <0.25 Low, 0.25–0.5 Medium, >0.5 High) instead of a separately trained classifier, ensuring the two are always aligned
- **Stricter individual approval threshold** — the 0.78 threshold is optimal at the *portfolio* level but too permissive for individual live decisions; consider a dual-threshold system (0.5 for individual scoring, 0.78 for portfolio-level batch optimization)
...
- Mobile-responsive layouts already in place via Tailwind; add PWA support for offline portfolio health caching)**
